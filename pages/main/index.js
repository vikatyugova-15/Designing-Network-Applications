...
export class MainPage { 
    ...
    consumeNavigateRequest() {
        const raw = sessionStorage.getItem('main.navigateTo');
        if (!raw) return;

        sessionStorage.removeItem('main.navigateTo');
    ...
    navigateToServiceInList(servicePath) {
        const path = String(servicePath ?? '').trim();
        if (!path) {
            this.fetchData(this.currentQuery, this.currentPage);
            return;
        }

        // Берём список целиком (для лабораторной это ок) и вычисляем страницу по текущему limit.
        ajax.get(serviceUrls.getServices('', 1, 10000), (data, status) => {
            if (status !== 200 || !data || !Array.isArray(data.items)) {
                this.fetchData('', 1);
                return;
            }

            const index = data.items.findIndex((s) => String(s?.path ?? '') === path);
            if (index < 0) {
                this.fetchData('', 1);
                return;
            }

            const targetPage = Math.floor(index / this.limit) + 1;
            this.pendingScrollToPath = path;
            this.searchDraft = '';
            this.fetchData('', targetPage);
        });
    }
    ...
    renderCards() {
        ...
        if (this.pendingScrollToPath) {
            const target = this.services?.find((s) => String(s?.path ?? '') === this.pendingScrollToPath);
            if (target?.id) {
    ...
    render() { 
        ...
        this.consumeNavigateRequest();

        if (this.pendingNavigatePath) {
            const path = this.pendingNavigatePath;
            this.pendingNavigatePath = null;
            this.navigateToServiceInList(path);
            return;
        }
